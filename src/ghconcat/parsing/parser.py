...
def _build_parser() -> argparse.ArgumentParser:
    ...
    Notes:
        - This parser defines only the effective flags required by the runtime.
        - Help texts are aligned with the current (v2) semantics and mapped from
          legacy documentation where applicable. No extra or deprecated flags
          are introduced here.
    """
    ...
    g_loc.add_argument(
        ...
        help=(
            "Workspace folder for templates, prompts, AI artefacts and outputs. "
            "Defaults to the current -w directory. Paths given to -o/-t/--ai-* "
            "are resolved here, keeping sources and generated files separated."
        ),
    )
    g_loc.add_argument(
        ...
        help=(
            ...
            "Bare CLI tokens that do not start with '-' are implicitly converted to this flag "
            "(e.g. `ghconcat src utils` becomes `-a src -a utils`)."
        ),
    )
    g_loc.add_argument(
    ...
    g_loc.add_argument(
        "--url-depth", metavar="N", type=int, dest="url_depth", default=0,
        help=(
            "Depth for URL crawling (0 fetch-only, >0 breadth-first scrape). "
            "Tokens that look like URLs are auto-classified; use this to control recursion."
        ),
    )
    g_loc.add_argument(
        "--url-allow-cross-domain", action="store_true", dest="url_cross_domain",
        help=(
    ...
    g_loc.add_argument(
        ...
        help=(
            ...
            "filter becomes positive (other extensions are ignored unless explicitly "
    ...
    g_rng.add_argument(
        "-m", "--keep-first-line", dest="first_flags", action="append_const", const="keep",
        help=(
    ...
    g_rng.add_argument(
        "-M", "--no-first-line", dest="first_flags", action="append_const", const="drop",
    ...
    # -----------------------
    g_sub.add_argument(
        "-y", "--replace", metavar="SPEC", action="append", dest="replace_rules",
        help=(
    ...
    g_sub.add_argument(
        "-Y", "--preserve", metavar="SPEC", action="append", dest="preserve_rules",
        help=(
    ...
    # -----------------------
    g_cln.add_argument(
        "-c", "--remove-comments", action="store_true", dest="rm_comments",
        help=(
            "Enable comment removal. Language-aware strippers (e.g. Python/Dart) are used "
            "when available. Use -C to cancel in a child context."
        ),
    )
    g_cln.add_argument(
        "-C", "--no-remove-comments", action="store_true", dest="no_rm_comments",
        help="Disable comment removal in this context (cancels -c).",
    )
    g_cln.add_argument(
    ...
    g_cln.add_argument(
        "-b", "--strip-blank", dest="blank_flags", action="append_const", const="strip",
        help="Delete blank lines left after cleaning.",
    )
    g_cln.add_argument(
        "-B", "--keep-blank", dest="blank_flags", action="append_const", const="keep",
    ...
    g_tpl.add_argument(
        "-h", "--header", dest="hdr_flags", action="append_const", const="show",
        help="Emit a banner header before each new file (`===== path =====`).",
    )
    g_tpl.add_argument(
        "-H", "--no-headers", dest="hdr_flags", action="append_const", const="hide",
        help="Suppress banner headers in this scope (child contexts may re-enable).",
    )
    g_tpl.add_argument(
        "-r", "--relative-path", dest="path_flags", action="append_const", const="relative",
        help="Show header paths relative to the current workdir (default).",
    )
    g_tpl.add_argument(
        "-R", "--absolute-path", dest="path_flags", action="append_const", const="absolute",
    ...
    # -----------------------
    g_misc.add_argument(
        "--preserve-cache", action="store_true",
    ...
    g_misc.add_argument(
        "--json-logs", action="store_true", dest="json_logs",
    ...
    g_misc.add_argument(
        "--classifier", metavar="REF", dest="classifier_ref",
        help=(
            'Custom classifier as "module.path:ClassName" or "none". '
            "If omitted, the default classifier is used. You may also set "
            "GHCONCAT_CLASSIFIER env var."
        ),
    )
    g_misc.add_argument(
        "--classifier-policies", metavar="NAME", dest="classifier_policies",
