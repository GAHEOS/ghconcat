...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # Cleaning flags (normalized semantics: -C cancels -c)
    g_cln.add_argument(
        "-c",
        "--remove-comments",
        action="store_true",
    ...
    g_cln.add_argument(
        "-C",
        "--no-remove-comments",
        "--remove-all-comments",  # legacy alias kept for backward compatibility
        action="store_true",
        dest="no_rm_comments",
    ...
    g_ai.add_argument(
        "--ai-reasoning-effort", metavar="LEVEL", dest="ai_reasoning_effort", choices=("low", "medium", "high")
    )
...
def _apply_replacements(
    text: str, replace_specs: Sequence[str] | None, preserve_specs: Sequence[str] | None
) -> str:
...
def _merge_ns(parent: argparse.Namespace, child: argparse.Namespace) -> argparse.Namespace:
    """
    Merge two argparse namespaces.

    IMPORTANT:
    - Comment flags precedence is resolved at the content-processing layer
      (WalkerAppender via `should_remove_comments`), where '-C' cancels '-c'.
      Here we keep a simple merge-by-kind to avoid side effects.
    """
