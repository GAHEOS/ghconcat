"""
execution – Orchestrated execution engine for ghconcat contexts.
"""
...
class ExecutionEngine:
    """High-level coordinator for executing a parsed directive tree.
    ...
    def execute_node(
        ...
        *,
        ...
        """Execute *node* and return (vars_map, final_output)."""
        ...
        try:
            ...
            if (
                ...
                scraped_files = self._discovery.scrape_urls(
                    ...
                    exclude_suf=exclude_suf,
                    max_depth=(
                        2 if getattr(ns_effective, "url_scrape_depth", None) is None
                        else ns_effective.url_scrape_depth
                    ),
                    same_host_only=not getattr(ns_effective, "disable_url_domain_only", False),
                )
