...
Refactor note:
- Now reuses `build_default_engine_config(...)` to eliminate duplication
  while keeping the exact same behavior and test coverage.
"""
...
class EngineRunner(ExecutionEngineProtocol):
    ...
    def _to_argv(self, ctx: ContextConfig) -> List[str]:
        from ghconcat.runtime.flag_mapping import context_to_argv
